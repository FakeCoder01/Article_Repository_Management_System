...
if (isset($_POST["query"])) {
  ...
  $query = "SELECT DISTINCT * FROM `ARMS`.`posts` WHERE `posts`.`title` LIKE '%$search%' 
            UNION
            SELECT DISTINCT * FROM `ARMS`.`posts` WHERE `posts`.`content` LIKE '%$search%'
            UNION
            SELECT DISTINCT * FROM `ARMS`.`posts` WHERE `posts`.`username` LIKE '%$search%'
            UNION
            SELECT DISTINCT * FROM `ARMS`.`posts` WHERE `posts`.`id` LIKE '%$search%'
            ORDER BY `title` ; 
            ";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo "<p>No mathing result for your search</p>";
  }
}
?>
